...
def remove_interview(
    interview_id: str = typer.Argument(..., help="Interview ID to remove (e.g., interview_001)"),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation prompt"),
    keep_source: bool = typer.Option(False, "--keep-source", help="Keep extracted audio files"),
) -> None:
    """Remove an interview and all associated data from the project.

    Permanently deletes the interview entry from the manifest and all related files.
    Project-level files (synthesis, selections, arc) will also be deleted and will
    need to be regenerated by re-running the pipeline.
    """
    project_dir = find_project_dir()
    if not project_dir:
        console.print("[red]Error: Not in a Plotline project directory[/red]")
    ...
    if not interview:
        console.print(f"[red]Error: Interview '{interview_id}' not found in project[/red]")
        console.print("[dim]Use 'plotline status' to see available interviews[/dim]")
        raise typer.Exit(1)

    files_to_delete: list[Path] = []
    bytes_to_free = 0

    source_dir = project_dir / "source" / interview_id
    if source_dir.exists() and not keep_source:
        for f in source_dir.rglob("*"):
            if f.is_file():
                files_to_delete.append(f)
                bytes_to_free += f.stat().st_size
        files_to_delete.append(source_dir)

    data_dirs = ["transcripts", "delivery", "segments", "themes", "diarization"]
    for data_dir in data_dirs:
        file_path = project_dir / "data" / data_dir / f"{interview_id}.json"
        if file_path.exists():
            files_to_delete.append(file_path)
            bytes_to_free += file_path.stat().st_size

    report_path = project_dir / "reports" / f"transcript_{interview_id}.html"
    if report_path.exists():
        files_to_delete.append(report_path)
        bytes_to_free += report_path.stat().st_size

    project_files = [
        project_dir / "data" / "synthesis.json",
        project_dir / "data" / "selections.json",
        project_dir / "data" / "arc.json",
    ]
    project_files_exist = [f for f in project_files if f.exists()]

    console.print(
        f"\n[yellow]⚠[/yellow]  This will permanently delete [cyan]{interview_id}[/cyan] and all associated data:\n"
    )
        ...
        for f in files_to_delete[:10]:
            if f.is_file():
                size_str = (
                    ...
                    else f"{f.stat().st_size / 1024:.1f} KB"
                )
                console.print(f"  {f.relative_to(project_dir)} [dim]({size_str})[/dim]")
            else:
    ...
    if project_files_exist:
        console.print(
            "\n[bold]Project-level files to delete (will need to re-run pipeline):[/bold]"
        )
        for f in project_files_exist:
            console.print(f"  {f.relative_to(project_dir)}")

    if keep_source and source_dir.exists():
        console.print(f"\n[dim]Keeping source audio: {source_dir.relative_to(project_dir)}/[/dim]")
    ...
    manifest["interviews"] = [
        i for i in manifest.get("interviews", []) if i.get("id") != interview_id
    ]
    project.save_manifest(manifest)

    if bytes_to_free > 1024 * 1024 * 1024:
        size_str = f"{bytes_to_free / 1024 / 1024 / 1024:.1f} GB"
    elif bytes_to_free > 1024 * 1024:
        size_str = f"{bytes_to_free / 1024 / 1024:.1f} MB"
    else:
        size_str = f"{bytes_to_free / 1024:.1f} KB"

    console.print(f"\n[green]✓[/green] Removed {interview_id} ({size_str} freed)")

    if project_files_exist:
        console.print(
            "[dim]Re-run 'plotline run --from themes' to regenerate project-level files[/dim]"
        )
